...
local CapsuleEntity = {}
CapsuleEntity.__index = CapsuleEntity


function CapsuleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CapsuleEntity)
  self._name = "capsule"
  self._client = client
...
function CapsuleEntity:get_name()
  return self._name
end


function CapsuleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CapsuleEntity.new(self._client, opts)
end
...
function CapsuleEntity:mark_deleted()
  self._deleted = true
end


function CapsuleEntity:deleted()
  return true == self._deleted
end


function CapsuleEntity:data_set(args)
  if args ~= nil then
...
function CapsuleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CapsuleEntity:match_set(args)
  if args ~= nil then
...
function CapsuleEntity:match_get()
...
function CapsuleEntity:stream(action, args, callopts)
...
function CapsuleEntity:_run_op(ctx, post_done)
  local utility = self._utility
