...
local LaunchpadEntity = {}
LaunchpadEntity.__index = LaunchpadEntity


function LaunchpadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LaunchpadEntity)
  self._name = "launchpad"
  self._client = client
...
function LaunchpadEntity:get_name()
  return self._name
end


function LaunchpadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LaunchpadEntity.new(self._client, opts)
end
...
function LaunchpadEntity:mark_deleted()
  self._deleted = true
end


function LaunchpadEntity:deleted()
  return true == self._deleted
end


function LaunchpadEntity:data_set(args)
  if args ~= nil then
...
function LaunchpadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LaunchpadEntity:match_set(args)
  if args ~= nil then
...
function LaunchpadEntity:match_get()
...
function LaunchpadEntity:stream(action, args, callopts)
...
function LaunchpadEntity:_run_op(ctx, post_done)
  local utility = self._utility
