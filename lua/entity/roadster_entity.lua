...
local RoadsterEntity = {}
RoadsterEntity.__index = RoadsterEntity


function RoadsterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RoadsterEntity)
  self._name = "roadster"
  self._client = client
...
function RoadsterEntity:get_name()
  return self._name
end


function RoadsterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RoadsterEntity.new(self._client, opts)
end
...
function RoadsterEntity:mark_deleted()
  self._deleted = true
end


function RoadsterEntity:deleted()
  return true == self._deleted
end


function RoadsterEntity:data_set(args)
  if args ~= nil then
...
function RoadsterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RoadsterEntity:match_set(args)
  if args ~= nil then
...
function RoadsterEntity:match_get()
...
function RoadsterEntity:stream(action, args, callopts)
...
function RoadsterEntity:_run_op(ctx, post_done)
  local utility = self._utility
