...
local LandpadEntity = {}
LandpadEntity.__index = LandpadEntity


function LandpadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LandpadEntity)
  self._name = "landpad"
  self._client = client
...
function LandpadEntity:get_name()
  return self._name
end


function LandpadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LandpadEntity.new(self._client, opts)
end
...
function LandpadEntity:mark_deleted()
  self._deleted = true
end


function LandpadEntity:deleted()
  return true == self._deleted
end


function LandpadEntity:data_set(args)
  if args ~= nil then
...
function LandpadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LandpadEntity:match_set(args)
  if args ~= nil then
...
function LandpadEntity:match_get()
...
function LandpadEntity:stream(action, args, callopts)
...
function LandpadEntity:_run_op(ctx, post_done)
  local utility = self._utility
