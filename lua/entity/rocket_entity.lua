...
local RocketEntity = {}
RocketEntity.__index = RocketEntity


function RocketEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RocketEntity)
  self._name = "rocket"
  self._client = client
...
function RocketEntity:get_name()
  return self._name
end


function RocketEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RocketEntity.new(self._client, opts)
end
...
function RocketEntity:mark_deleted()
  self._deleted = true
end


function RocketEntity:deleted()
  return true == self._deleted
end


function RocketEntity:data_set(args)
  if args ~= nil then
...
function RocketEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RocketEntity:match_set(args)
  if args ~= nil then
...
function RocketEntity:match_get()
...
function RocketEntity:stream(action, args, callopts)
...
---@param reqmatch RocketLoadMatch
---@param ctrl? table
---@return Rocket
---@return string? err
function RocketEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch RocketListMatch
---@param ctrl? table
---@return Rocket[]
---@return string? err
function RocketEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function RocketEntity:_run_op(ctx, post_done)
  local utility = self._utility
