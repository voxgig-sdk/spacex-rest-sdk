...
local StarlinkEntity = {}
StarlinkEntity.__index = StarlinkEntity


function StarlinkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StarlinkEntity)
  self._name = "starlink"
  self._client = client
...
function StarlinkEntity:get_name()
  return self._name
end


function StarlinkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StarlinkEntity.new(self._client, opts)
end
...
function StarlinkEntity:mark_deleted()
  self._deleted = true
end


function StarlinkEntity:deleted()
  return true == self._deleted
end


function StarlinkEntity:data_set(args)
  if args ~= nil then
...
function StarlinkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StarlinkEntity:match_set(args)
  if args ~= nil then
...
function StarlinkEntity:match_get()
...
function StarlinkEntity:stream(action, args, callopts)
...
function StarlinkEntity:_run_op(ctx, post_done)
  local utility = self._utility
