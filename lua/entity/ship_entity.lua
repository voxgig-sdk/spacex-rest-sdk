...
function ShipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShipEntity)
  self._name = "ship"
  self._client = client
...
function ShipEntity:get_name()
  return self._name
end


function ShipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShipEntity.new(self._client, opts)
end
...
function ShipEntity:mark_deleted()
  self._deleted = true
end


function ShipEntity:deleted()
  return true == self._deleted
end


function ShipEntity:data_set(args)
  if args ~= nil then
...
function ShipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShipEntity:match_set(args)
  if args ~= nil then
...
function ShipEntity:stream(action, args, callopts)
...
function ShipEntity:_run_op(ctx, post_done)
  local utility = self._utility
