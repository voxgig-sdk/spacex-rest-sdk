...
local PayloadEntity = {}
PayloadEntity.__index = PayloadEntity


function PayloadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PayloadEntity)
  self._name = "payload"
  self._client = client
...
function PayloadEntity:get_name()
  return self._name
end


function PayloadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PayloadEntity.new(self._client, opts)
end
...
function PayloadEntity:mark_deleted()
  self._deleted = true
end


function PayloadEntity:deleted()
  return true == self._deleted
end


function PayloadEntity:data_set(args)
  if args ~= nil then
...
function PayloadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PayloadEntity:match_set(args)
  if args ~= nil then
...
function PayloadEntity:match_get()
...
function PayloadEntity:stream(action, args, callopts)
...
function PayloadEntity:_run_op(ctx, post_done)
  local utility = self._utility
