...
local LaunchEntity = {}
LaunchEntity.__index = LaunchEntity


function LaunchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LaunchEntity)
  self._name = "launch"
  self._client = client
...
function LaunchEntity:get_name()
  return self._name
end


function LaunchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LaunchEntity.new(self._client, opts)
end
...
function LaunchEntity:mark_deleted()
  self._deleted = true
end


function LaunchEntity:deleted()
  return true == self._deleted
end


function LaunchEntity:data_set(args)
  if args ~= nil then
...
function LaunchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LaunchEntity:match_set(args)
  if args ~= nil then
...
function LaunchEntity:match_get()
...
function LaunchEntity:stream(action, args, callopts)
...
---@param reqmatch LaunchLoadMatch
---@param ctrl? table
---@return Launch
---@return string? err
function LaunchEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch LaunchListMatch
---@param ctrl? table
---@return Launch[]
---@return string? err
function LaunchEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function LaunchEntity:_run_op(ctx, post_done)
  local utility = self._utility
