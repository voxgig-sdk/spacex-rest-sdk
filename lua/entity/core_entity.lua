...
local CoreEntity = {}
CoreEntity.__index = CoreEntity


function CoreEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CoreEntity)
  self._name = "core"
  self._client = client
...
function CoreEntity:get_name()
  return self._name
end


function CoreEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CoreEntity.new(self._client, opts)
end
...
function CoreEntity:mark_deleted()
  self._deleted = true
end


function CoreEntity:deleted()
  return true == self._deleted
end


function CoreEntity:data_set(args)
  if args ~= nil then
...
function CoreEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CoreEntity:match_set(args)
  if args ~= nil then
...
function CoreEntity:stream(action, args, callopts)
...
---@param reqmatch CoreListMatch
---@param ctrl? table
---@return Core[]
---@return string? err
function CoreEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CoreEntity:_run_op(ctx, post_done)
  local utility = self._utility
