...
local CrewEntity = {}
CrewEntity.__index = CrewEntity


function CrewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CrewEntity)
  self._name = "crew"
  self._client = client
...
function CrewEntity:get_name()
  return self._name
end


function CrewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CrewEntity.new(self._client, opts)
end
...
function CrewEntity:mark_deleted()
  self._deleted = true
end


function CrewEntity:deleted()
  return true == self._deleted
end


function CrewEntity:data_set(args)
  if args ~= nil then
...
function CrewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CrewEntity:match_set(args)
  if args ~= nil then
...
function CrewEntity:stream(action, args, callopts)
...
function CrewEntity:_run_op(ctx, post_done)
  local utility = self._utility
