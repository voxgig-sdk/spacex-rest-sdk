...
describe("RocketDirect", function()
  it("should direct-list-rocket", function()
    local setup = rocket_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-rocket", setup.live and "live" or "unit")
  ...
  it("should direct-load-rocket", function()
    local setup = rocket_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-rocket", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function rocket_direct_setup(mockres)
