...
describe("LaunchpadEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Launchpad(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Launchpad(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Launchpad(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = launchpad_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "launchpad." .. _op, _live and "live" or "unit")
    ...
    local launchpad_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.launchpad")))
    local launchpad_ref01_data = nil
    if #launchpad_ref01_data_raw > 0 then
      launchpad_ref01_data = helpers.to_map(launchpad_ref01_data_raw[1][2])
    end

    -- LIST
    local launchpad_ref01_ent = client:Launchpad(nil)
    local launchpad_ref01_match = {}

    local launchpad_ref01_list_result, err = launchpad_ref01_ent:list(launchpad_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(launchpad_ref01_list_result)

    -- LOAD
    local launchpad_ref01_match_dt0 = {
...
function launchpad_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/launchpad/LaunchpadTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read launchpad test data: " .. entity_data_file)
  end
