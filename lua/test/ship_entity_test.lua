...
describe("ShipEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Ship(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Ship(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_SHIP_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local ship_ref01_data_dt0_load_result = helpers.to_map(type(ship_ref01_data_dt0_loaded) == 'table' and ship_ref01_data_dt0_loaded.data_get and ship_ref01_data_dt0_loaded:data_get() or ship_ref01_data_dt0_loaded)
...
function ship_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SPACEX_REST_TEST_SHIP_ENTID")
