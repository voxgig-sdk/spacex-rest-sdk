...
describe("CoreDirect", function()
  it("should direct-list-core", function()
    local setup = core_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-core", setup.live and "live" or "unit")
  ...
  it("should direct-load-core", function()
    local setup = core_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-core", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function core_direct_setup(mockres)
