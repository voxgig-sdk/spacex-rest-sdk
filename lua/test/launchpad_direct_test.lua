...
describe("LaunchpadDirect", function()
  it("should direct-list-launchpad", function()
    local setup = launchpad_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-launchpad", setup.live and "live" or "unit")
  ...
  it("should direct-load-launchpad", function()
    local setup = launchpad_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-launchpad", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function launchpad_direct_setup(mockres)
