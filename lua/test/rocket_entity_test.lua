...
describe("RocketEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_ROCKET_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local rocket_ref01_data_dt0_load_result = helpers.to_map(type(rocket_ref01_data_dt0_loaded) == 'table' and rocket_ref01_data_dt0_loaded.data_get and rocket_ref01_data_dt0_loaded:data_get() or rocket_ref01_data_dt0_loaded)
...
function rocket_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SPACEX_REST_TEST_ROCKET_ENTID")
