...
describe("PayloadDirect", function()
  it("should direct-list-payload", function()
    local setup = payload_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-payload", setup.live and "live" or "unit")
  ...
  it("should direct-load-payload", function()
    local setup = payload_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-payload", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function payload_direct_setup(mockres)
