...
describe("ShipDirect", function()
  it("should direct-list-ship", function()
    local setup = ship_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-ship", setup.live and "live" or "unit")
  ...
  it("should direct-load-ship", function()
    local setup = ship_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-ship", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function ship_direct_setup(mockres)
