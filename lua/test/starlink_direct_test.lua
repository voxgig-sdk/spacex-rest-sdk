...
describe("StarlinkDirect", function()
  it("should direct-list-starlink", function()
    local setup = starlink_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-starlink", setup.live and "live" or "unit")
  ...
  it("should direct-load-starlink", function()
    local setup = starlink_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-starlink", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function starlink_direct_setup(mockres)
