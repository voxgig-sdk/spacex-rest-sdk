...
describe("CrewDirect", function()
  it("should direct-list-crew", function()
    local setup = crew_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-crew", setup.live and "live" or "unit")
  ...
  it("should direct-load-crew", function()
    local setup = crew_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-crew", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function crew_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["SPACEX_REST_TEST_CREW_ENTID"] = {},
