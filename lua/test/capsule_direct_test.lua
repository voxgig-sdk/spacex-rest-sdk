...
describe("CapsuleDirect", function()
  it("should direct-list-capsule", function()
    local setup = capsule_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-capsule", setup.live and "live" or "unit")
  ...
  it("should direct-load-capsule", function()
    local setup = capsule_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-capsule", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function capsule_direct_setup(mockres)
