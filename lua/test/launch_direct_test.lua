...
describe("LaunchDirect", function()
  it("should direct-list-launch", function()
    local setup = launch_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-launch", setup.live and "live" or "unit")
  ...
  it("should direct-load-launch", function()
    local setup = launch_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-launch", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function launch_direct_setup(mockres)
