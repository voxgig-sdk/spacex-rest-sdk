...
The Lua SDK for the SpacexRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Capsule(nil):list(nil, nil)
...
```lua
local result, err = client:Capsule(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:SpacexRest(nil):load(
  { id = "test01" }, nil
)
