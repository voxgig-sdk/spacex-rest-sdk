...
-- by passing it.
function SpacexRestSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function SpacexRestSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "SpacexRestSDK: graphql: " .. msg
    res.graphql = errors
  end
