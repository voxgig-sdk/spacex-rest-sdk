...
describe('LandpadDirect', async () => {
  ...
  test('direct-load-landpad', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-landpad', setup.live)) return
  ...
  test('direct-list-landpad', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-landpad', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SPACEXREST_TEST_LANDPAD_ENTID']
