...
describe('LaunchpadDirect', async () => {
  ...
  test('direct-load-launchpad', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-launchpad', setup.live)) return
  ...
  test('direct-list-launchpad', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-launchpad', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  const live = 'TRUE' === env.SPACEXREST_TEST_LIVE
  ...
  if (live) {
    ...
    let idmap: any = env['SPACEXREST_TEST_LAUNCHPAD_ENTID']
