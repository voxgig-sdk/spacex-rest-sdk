...
describe('PayloadDirect', async () => {
  ...
  test('direct-load-payload', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-payload', setup.live)) return
  ...
  test('direct-list-payload', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-payload', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SPACEXREST_TEST_PAYLOAD_ENTID']
