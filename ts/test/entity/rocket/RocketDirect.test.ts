...
describe('RocketDirect', async () => {
  ...
  test('direct-load-rocket', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-rocket', setup.live)) return
  ...
  test('direct-list-rocket', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-rocket', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SPACEXREST_TEST_ROCKET_ENTID']
