...
//     (\`new SpacexRestSDK(...)\` / \`SpacexRestSDK.test(...)\` become
//     \`SpacexRestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"capsule":{"test01":{"id":"test01"}},"core":{"test01":{"id":"test01"}},"crew":{"test01":{"id":"test01"}},"landpad":{"test01":{"id":"test01"}},"launch":{"test01":{"id":"test01"}},"launchpad":{"test01":{"id":"test01"}},"payload":{"test01":{"id":"test01"}},"roadster":{"test01":{"id":"test01"}},"rocket":{"test01":{"id":"test01"}},"ship":{"test01":{"id":"test01"}},"starlink":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SpacexRestSDK(...)\` and any
// \`SpacexRestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SpacexRestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SpacexRestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
