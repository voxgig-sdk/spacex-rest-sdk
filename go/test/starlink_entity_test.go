...
func TestStarlinkEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Starlink(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Starlink(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_STARLINK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		starlinkRef01DataDt0LoadResult := core.ToMapAny(entityData(starlinkRef01DataDt0Loaded))
...
func starlinkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPACEX_REST_TEST_STARLINK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPACEX_REST_TEST_STARLINK_ENTID"])
