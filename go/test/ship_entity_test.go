...
func TestShipEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Ship(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Ship(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		shipRef01DataDt0LoadResult := core.ToMapAny(entityData(shipRef01DataDt0Loaded))
...
func shipBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPACEX_REST_TEST_SHIP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPACEX_REST_TEST_SHIP_ENTID"])
