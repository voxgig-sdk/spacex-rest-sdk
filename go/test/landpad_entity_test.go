...
func TestLandpadEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Landpad(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Landpad(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_LANDPAD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		landpadRef01DataDt0LoadResult := core.ToMapAny(entityData(landpadRef01DataDt0Loaded))
...
func landpadBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPACEX_REST_TEST_LANDPAD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPACEX_REST_TEST_LANDPAD_ENTID"])
