...
func TestCoreEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SPACEXREST_TEST_CORE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		coreRef01DataDt0LoadResult := core.ToMapAny(coreRef01DataDt0Loaded)
...
func coreBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPACEXREST_TEST_CORE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPACEXREST_TEST_CORE_ENTID"])
