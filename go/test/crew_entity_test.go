...
func TestCrewEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Crew(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Crew(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_CREW_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		// LOAD
		crewRef01MatchDt0 := map[string]any{
			"id": crewRef01Data["id"],
		}
		crewRef01DataDt0Loaded, err := crewRef01Ent.Load(crewRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		crewRef01DataDt0LoadResult := core.ToMapAny(entityData(crewRef01DataDt0Loaded))
		if crewRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if crewRef01DataDt0LoadResult["id"] != crewRef01Data["id"] {
...
func crewBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPACEX_REST_TEST_CREW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPACEX_REST_TEST_CREW_ENTID"])
