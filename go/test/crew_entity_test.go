...
func TestCrewEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SPACEXREST_TEST_CREW_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		crewRef01DataDt0LoadResult := core.ToMapAny(crewRef01DataDt0Loaded)
...
func crewBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SPACEXREST_TEST_CREW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SPACEXREST_TEST_CREW_ENTID"])
