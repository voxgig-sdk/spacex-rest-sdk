...
The Python SDK for the SpacexRest API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Capsule(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Capsule(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.SpacexRest(None).load(
    {"id": "test01"}, None
)
