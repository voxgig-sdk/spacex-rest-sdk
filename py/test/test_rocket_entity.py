...
class TestRocketEntity:

    def test_should_create_instance(self):
        testsdk = SpacexRestSDK.test(None, None)
        ent = testsdk.Rocket(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Rocket(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Rocket(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _rocket_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "rocket." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SPACEX_REST_TEST_ROCKET_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        rocket_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.rocket")))
        rocket_ref01_data = None
        if len(rocket_ref01_data_raw) > 0:
        ...
        rocket_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(rocket_ref01_data_dt0_loaded))
        assert rocket_ref01_data_dt0_load_result is not None
        assert rocket_ref01_data_dt0_load_result["id"] == rocket_ref01_data["id"]



def _rocket_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/rocket/RocketTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SPACEX_REST_TEST_ROCKET_ENTID"))
