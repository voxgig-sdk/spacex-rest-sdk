...
class SpacexRestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SpacexRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SpacexRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SpacexRestTestRunner.getenv("SPACEX_REST_TEST_LIVE")
        override = SpacexRestTestRunner.getenv("SPACEX_REST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SpacexRestTestRunner.getenv(key)
        ...
        explain = SpacexRestTestRunner.getenv("SPACEX_REST_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SpacexRestTestRunner._test_control is not None:
            return SpacexRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SpacexRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SpacexRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SpacexRestTestRunner.load_test_control()
...
def load_env_local():
    SpacexRestTestRunner.load_env_local()


def env_override(m):
    return SpacexRestTestRunner.env_override(m)


def entity_data(v):
    return SpacexRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SpacexRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SpacexRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SpacexRestTestRunner.load_test_control()


def live_delay_ms():
    return SpacexRestTestRunner.live_delay_ms()
