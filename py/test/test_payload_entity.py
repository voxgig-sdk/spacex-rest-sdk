...
class TestPayloadEntity:

    def test_should_create_instance(self):
        testsdk = SpacexRestSDK.test(None, None)
        ent = testsdk.Payload(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Payload(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Payload(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _payload_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "payload." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SPACEX_REST_TEST_PAYLOAD_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        payload_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.payload")))
        payload_ref01_data = None
        if len(payload_ref01_data_raw) > 0:
        ...
        payload_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(payload_ref01_data_dt0_loaded))
        assert payload_ref01_data_dt0_load_result is not None
        assert payload_ref01_data_dt0_load_result["id"] == payload_ref01_data["id"]



def _payload_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/payload/PayloadTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "SPACEX_REST_TEST_PAYLOAD_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SPACEX_REST_TEST_PAYLOAD_ENTID"))
