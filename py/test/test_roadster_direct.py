...
class TestRoadsterDirect:

    def test_should_direct_list_roadster(self):
        setup = _roadster_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-roadster", "live" if setup["live"] else "unit")
        if _skip:
...
def _roadster_direct_setup(mockres):
