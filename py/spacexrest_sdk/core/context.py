...
from spacexrest_sdk.core.helpers import get_ctx_prop, to_map
...
class SpacexRestContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, SpacexRestSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, SpacexRestResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, SpacexRestResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return SpacexRestError(code, msg, self)
