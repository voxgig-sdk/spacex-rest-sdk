...
// SpacexRest SDK Launchpad entity
...
class LaunchpadEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LaunchpadEntity($this->_client, $opts);
    }

    /**
     * @param Launchpad|array $args Launchpad data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = SpacexRestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Launchpad|array The current Launchpad data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Launchpad fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = SpacexRestHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Launchpad fields).
     */
    ...
    /**
     * Load a single Launchpad.
     *
     * @param LaunchpadLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed LaunchpadLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Launchpad|array The loaded Launchpad as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = SpacexRestHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Launchpad items matching the given filter.
     *
     * @param LaunchpadListMatch|array|null $reqmatch Match filter (any subset
     *   of Launchpad fields) as an assoc-array; LaunchpadListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Launchpad[]|array A list of Launchpad items as assoc-arrays at
