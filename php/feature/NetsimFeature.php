...
class SpacexRestNetsimFeature extends SpacexRestBaseFeature
{
    ...
    private function _simulate(SpacexRestContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(SpacexRestContext $ctx, array $applied): void
    {
        $client = $this->client;
