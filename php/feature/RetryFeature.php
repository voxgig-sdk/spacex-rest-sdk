...
class SpacexRestRetryFeature extends SpacexRestBaseFeature
{
    ...
    public function init(SpacexRestContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SpacexRestContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SpacexRestContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SpacexRestContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
