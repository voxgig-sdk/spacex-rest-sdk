...
class LaunchEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = SpacexRestSDK::test(null, null);
        $ent = $testsdk->Launch(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Launch(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Launch(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = launch_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "launch." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_LAUNCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $launch_ref01_data = null;
        if (count($launch_ref01_data_raw) > 0) {
        ...
        $launch_ref01_data_dt0_load_result = Helpers::to_map(is_object($launch_ref01_data_dt0_loaded) && method_exists($launch_ref01_data_dt0_loaded, 'data_get') ? $launch_ref01_data_dt0_loaded->data_get() : $launch_ref01_data_dt0_loaded);
...
function launch_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/launch/LaunchTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["launch01", "launch02", "launch03"] as $k) {
    ...
    $entid_env_raw = getenv("SPACEX_REST_TEST_LAUNCH_ENTID");
