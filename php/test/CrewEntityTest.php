...
class CrewEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = SpacexRestSDK::test(null, null);
        $ent = $testsdk->Crew(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Crew(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Crew(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = crew_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "crew." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_CREW_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $crew_ref01_data = null;
        if (count($crew_ref01_data_raw) > 0) {
        ...
        $crew_ref01_data_dt0_load_result = Helpers::to_map(is_object($crew_ref01_data_dt0_loaded) && method_exists($crew_ref01_data_dt0_loaded, 'data_get') ? $crew_ref01_data_dt0_loaded->data_get() : $crew_ref01_data_dt0_loaded);
...
function crew_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/crew/CrewTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["crew01", "crew02", "crew03"] as $k) {
    ...
    $entid_env_raw = getenv("SPACEX_REST_TEST_CREW_ENTID");
