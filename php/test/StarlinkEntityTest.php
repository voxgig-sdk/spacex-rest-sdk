...
class StarlinkEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_STARLINK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $starlink_ref01_data_dt0_load_result = Helpers::to_map(is_object($starlink_ref01_data_dt0_loaded) && method_exists($starlink_ref01_data_dt0_loaded, 'data_get') ? $starlink_ref01_data_dt0_loaded->data_get() : $starlink_ref01_data_dt0_loaded);
...
function starlink_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("SPACEX_REST_TEST_STARLINK_ENTID");
