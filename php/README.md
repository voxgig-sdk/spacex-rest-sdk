...
The PHP SDK for the SpacexRest API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Capsule(null)->list(null, null);
...
```php
[$result, $err] = $client->Capsule(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->SpacexRest(null)->load(
    ["id" => "test01"], null
);
