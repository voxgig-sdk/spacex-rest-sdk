...
class SpacexRestMakeError
{
    public static function call(?SpacexRestContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new SpacexRestContext([], null);
        }
        $op = $ctx->op ?? new SpacexRestOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new SpacexRestResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof SpacexRestError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new SpacexRestError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof SpacexRestError) {
