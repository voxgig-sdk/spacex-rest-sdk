...
[MCP](https://modelcontextprotocol.io) server exposing the SpacexRest SDK as
two agent tools — `spacex-rest_list` and `spacex-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/spacex-rest-mcp)
make build

# 2. Provide credentials via the environment
export SPACEX_REST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user spacex-rest \
  -- /absolute/path/to/spacex-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./spacex-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// spacex-rest_list: first page of records
{ "entity": "capsule" }
{ "entity": "capsule", "query": { } }

// spacex-rest_load: one record by id
{ "entity": "capsule", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/spacex-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user spacex-rest \
     -- "$PWD"/dist/*/spacex-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `spacex-rest_list` and `spacex-rest_load` tools now appear
   in new sessions. Ask the agent to *"list capsule using spacex-rest"*
   and it calls `spacex-rest_list` with `{"entity":"capsule"}`.
...
```sh
export SPACEX_REST_APIKEY=sk_live_xxx            # API key
export SPACEX_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `spacex-rest_list` tool
...
### Call the `spacex-rest_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `spacex-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `spacex-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SPACEX_REST_APIKEY` | API key sent with every request. |
| `SPACEX_REST_BASE` | Optional override of the API base URL. |
...
```sh
./spacex-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"spacex-rest_load","arguments":{"entity":"capsule","query":{"id":1}}}}'
```
