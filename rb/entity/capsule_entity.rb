...
class CapsuleEntity
  ...
  def make
    opts = @_entopts.dup
    CapsuleEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Capsule.load works with no args).
  ...
  #
  # @param reqmatch [CapsuleListMatch, Hash, nil] match filter (any subset of
  #   Capsule fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
