...
class CrewEntity
  ...
  # @return [Crew, Hash] the current Crew data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Crew fields)
  def match_get
  ...
  # Load a single Crew.
  #
  # @param reqmatch [CrewLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Crew.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Crew, Hash] the loaded Crew; raises SpacexRestError on failure
  ...
  #
  # @param reqmatch [CrewListMatch, Hash, nil] match filter (any subset of
  #   Crew fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Crew>, Array] the matching Crew items; raises SpacexRestError on failure
