...
class LaunchpadEntity
  ...
  def make
    opts = @_entopts.dup
    LaunchpadEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SpacexRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Launchpad, Hash] the current Launchpad data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SpacexRestHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Launchpad fields)
  def match_get
  ...
  #
  # @param reqmatch [LaunchpadLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Launchpad.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Launchpad, Hash] the loaded Launchpad; raises SpacexRestError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SpacexRestHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [LaunchpadListMatch, Hash, nil] match filter (any subset of
  #   Launchpad fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Launchpad>, Array] the matching Launchpad items; raises SpacexRestError on failure
