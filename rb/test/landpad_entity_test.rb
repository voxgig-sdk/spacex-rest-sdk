...
class LandpadEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Landpad(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Landpad(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = landpad_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "landpad." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    landpad_ref01_data = nil
    if landpad_ref01_data_raw.length > 0
      landpad_ref01_data = Helpers.to_map(landpad_ref01_data_raw[0][1])
    end

    # LIST
    landpad_ref01_ent = client.Landpad(nil)
    landpad_ref01_match = {}

    landpad_ref01_list_result = landpad_ref01_ent.list(landpad_ref01_match, nil)
    assert landpad_ref01_list_result.is_a?(Array)
...
def landpad_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "landpad", "LandpadTestData.json")
