...
class CoreDirectTest < Minitest::Test
  def test_direct_list_core
    ...
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-core", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_core
    setup = core_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-core", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
