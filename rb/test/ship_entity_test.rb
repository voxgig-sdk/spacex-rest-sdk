...
class ShipEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Ship(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Ship(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SPACEX_REST_TEST_SHIP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    ship_ref01_data_dt0_load_result = Helpers.to_map(ship_ref01_data_dt0_loaded.respond_to?(:data_get) ? ship_ref01_data_dt0_loaded.data_get : ship_ref01_data_dt0_loaded)
...
def ship_basic_setup(extra)
  ...
  entid_env_raw = ENV["SPACEX_REST_TEST_SHIP_ENTID"]
