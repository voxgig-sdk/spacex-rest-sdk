...
boru-driven command-line client **and** interactive REPL for the SpacexRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/spacex-rest-cli)
make build

# 2. See usage (words, entities, env vars)
./spacex-rest-cli --help
...
./spacex-rest-cli load 1 capsule            # {id:1} shorthand
./spacex-rest-cli load '{id:1}' capsule       # explicit match map
./spacex-rest-cli list core

# 5. Override the API base URL for a single call
SPACEX_REST_BASE=https://api.example.com ./spacex-rest-cli list capsule

# 6. No arguments -> interactive REPL
./spacex-rest-cli
spacex-rest> list capsule
spacex-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/spacex-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/spacex-rest-cli list capsule
   ```
...
```sh
./spacex-rest-cli load 1 capsule          # scalar shorthand for {id:1}
./spacex-rest-cli load '{id:1}' capsule     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `spacex-rest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `capsule`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/spacex-rest-cli`. |
...
(`./spacex-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
