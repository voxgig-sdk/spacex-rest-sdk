// Command SpacexRest-cli is a CLI + REPL for the SpacexRest SDK,
...
const prompt = "spacex-rest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "capsule core crew landpad launch launchpad payload roadster rocket ship starlink"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SPACEX_REST_APIKEY (api key), SPACEX_REST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SPACEX_REST_APIKEY carries the API key and
	// SPACEX_REST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SPACEX_REST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SPACEX_REST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSpacexRestSDK(opts)
