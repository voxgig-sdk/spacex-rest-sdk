...
class FtCtrl extends SpacexRestControl
{
...
class FtClient
{
    ...
    private SpacexRestUtility $utility;

    public function __construct(SpacexRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): SpacexRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SpacexRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SpacexRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SpacexRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SpacexRestConfig::make_feature($name);
    ...
    private static function buildUrl(SpacexRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SpacexRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SpacexRestResponse($fetched);
        }
    ...
    private function fail(SpacexRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SpacexRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
