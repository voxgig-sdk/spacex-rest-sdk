...
module SpacexRestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SpacexRestResponse.new({})
    ...
    if fetch_err
      ...
      response = SpacexRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SpacexRestResponse.new(fetched)
    else
